...
export default class ContractDatabaseRepository implements ContractRepository {
	...
	async list(): Promise<Contract[]> {
		...
		const contractsData = await this.connection.query("select * from yllebs.contract", []);
		for (const contractData of contractsData) {
			...
			const paymentsData = await this.connection.query("select * from yllebs.payment where id_contract = $1", [contract.idContract]);
